...
def RASD_PLOT_1(DATASET, PLOT_SETUP):
    """
    THIS FUNCTION PLOT BOXPLOT AND HISTOGRAMS IN A SINGLE GRAPHIC
    ...
    sns.histplot(DATASET, ax = AX_HIST, kde = KDE, color = CHART_COLOR)
    AX_BOX.set(yticks = [])
    AX_BOX.set(xlabel='')
    font = {'fontname': 'Arial',
            'color':  LABELS_COLOR,
...
def RASD_PLOT_2(DATASET, PLOT_SETUP):
    """
    THIS FUNCTION PLOT SCATTER PLOT R AND G PARAMETERS ABOUT I INDEX
